...
| Zone | MC Range | Percentile | Meaning |
|------|----------|------------|---------|
| Panic | < 12 | Bottom 3% | No edge exists |
| Fear | 12–30 | P5–P45 | Oversold opportunity |
| Normal | 30–55 | P45–P75 | Sweet spot for buying |
| Extended | 55–68 | P75–P90 | Market stretched |
| Euphoria | ≥ 68 | Top 10% | Risk of reversal |
...
|---|-----------|--------|----------|
| O1a | VIX ≥ 30 | Deploy 95% capital. SA 75+ stocks first, fill remainder with QQQ. | 6yr data: +6.65% avg 1M, 84.9% win (N=152 days) |
| O1b | VIX ≥ 25 | Deploy 80% capital. SA 75+ stocks first, fill remainder with QQQ. | 6yr data: +4.20% avg 1M, 74.6% win (N=351 days) |
| O2 | MC < 12 | All cash. Sell everything. Ignore SA. | True panic — forward returns near zero |
| O3 | MC ≥ 68 for 3+ consecutive days | No new buys. Set 5% stop on all positions. | Top decile. 65-80 zone: 55.6% win rate — fading edge |
...
|---|---------|---------------|--------|----------|
| B1 | Fear (12–30) | SA ≥ 75 | **BUY** | Aug-Sep backtests: +5-7% at 1M, +32-62% at 3M, 60-100% win |
| B2 | Normal (30–55) | SA ≥ 65 | **BUY** | 6yr data: +2.3% avg 1M, 74.7% win rate. Best risk-adjusted zone |
| B3 | Extended (55–68) | — | **NO NEW BUYS. Hold existing.** | Oct + Mar backtests: -3 to -4% avg 1M, 37-42% win rate |
| B4 | Euphoria (≥ 68) | — | **NO NEW BUYS. Hold existing.** | Declining edge, reversal risk |
...
| Fear Buy (MC 12-30) | Equal weight | Up to 5 stocks |
| Normal Buy (MC 30-55) | Equal weight | Up to 8 stocks |
...
| 65–74 | 10% trailing stop | **Hold with caution.** Set or adjust 10% trailing stop from recent high. |
| 55–64 | 5% trailing stop | **Hold tight.** Set or adjust 5% trailing stop from recent high. |
...
- **VIX spike above 22:** Re-score all holdings immediately.
...
| S3 | Override O2 triggers (MC < 12) | Sell everything | Immediate |
| S4 | Override O3 triggers (MC ≥ 68, 3+ days) | 5% stop on all — let stops manage exit | Set and wait |

### Discretionary Sells

| # | Situation | Guidance |
|---|-----------|----------|
| D1 | SA 65-74 and MC moves from Normal → Extended | Consider selling weaker positions first |
...
| Sold due to MC panic (O2) | Wait for MC to exit Panic zone (≥ 12) before any new buys |
...
START
  ...
  ├─ Check MC Zone
  │   ├─ MC < 12 → ALL CASH → DONE
  │   ├─ MC 12-30 → Buy SA ≥ 75 only
  │   ├─ MC 30-55 → Buy SA ≥ 65
  │   ├─ MC 55-68 → No new buys, hold existing
  │   └─ MC ≥ 68 (3+ days) → 5% stops on everything
  │
  ├─ For Holdings: Weekly Re-Score
  │   ├─ SA ≥ 75 → Hold, no stop
  │   ├─ SA 65-74 → 10% trailing stop
  │   ├─ SA 55-64 → 5% trailing stop
...
| Panic Cash (O2) | ⚠️ Rare | ~P3, very few occurrences |
